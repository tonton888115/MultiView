...
| YouTube | ✅ InnerTube HLS + AVPlayer/iframe fallback | △ needs Data API + OAuth | ✅ official live chat in focus (login) |
...
When an HLS URL is available, the app uses AVPlayer for native playback. If extraction fails or
playback does not start, it falls back to the official iframe player.
