...
const SOURCES: { key: Platform; label: string; url: string }[] = [
  { key: 'twitch', label: 'Twitch', url: 'https://m.twitch.tv/directory/following' },
  { key: 'youtube', label: 'YouTube', url: 'https://m.youtube.com/feed/subscriptions' },
...
export default function FollowingTab({ onAddStream }: Props) {
        ...
        {SOURCES.map(s => {
          ...
          return (
            <TouchableOpacity
              ...
              <Text style={[styles.tabText, on && styles.tabTextActive]}>{s.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
      <Text style={styles.hint}>各サービスにログインすると、フォロー中の配信をタップで追加できます</Text>
      <WebView
        key={active}
...
const styles = StyleSheet.create({
  ...
  hint: { color: '#888', fontSize: 11, paddingHorizontal: 12, paddingVertical: 6, backgroundColor: '#161616' },
